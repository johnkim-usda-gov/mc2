...
#define FSrand_seed FS_F2+1
#define FSyrs_in_sum_ann_ppt FSrand_seed+1
...
#define NUM_WS_OUTVARS (LAST_BS_OUTVAR_INDEX+1) 

#define NUMBER_OF_ZONES	(mapssTROPICAL + 1)
#define NZONES NUMBER_OF_ZONES

#define NLAYER_LE_9_FLAG 0 /* TRUE => limit nlayer to 9 for backward compatibility with some
      versions prior to 2B67 */
...
#define MASK_GE_997000_CODE_FLAG 6 /* Interpret mask values >= 997000 as cells to be skipped
...
typedef enum {
    ...
    MESIC_TEMPERATE_NEEDLELEAF_FORESTveg, // 8
    ...
    // XERIC_NEEDLELEAF_WOODLANDveg, // 50 dry pine woodland 
    } MC2VegType;
#define MAX_VTYPE DRY_TEMPERATE_NEEDLELEAF_FORESTveg
// #define MAX_VTYPE XERIC_NEEDLELEAF_WOODLANDveg
...
#define Lynx_Temperate_Evergreen_Needleleaf_Forest MESIC_TEMPERATE_NEEDLELEAF_FORESTveg // 8
...
#define Lynx_Temperate_Shrubland C3SHRUBveg // 16
...
#define Lynx_Subtropical_Shrubland C4SHRUBveg // 27
...
#define Lynx_Tropical_Shrubland VTYPE33veg // 33
...
class ModelParamsClass
{
   public:
      ...
      // Next 4 are coefficient sets for vegetation zone calculations in the WCR base calibration.
...
class InputDataClass
{
  public:
    ...
    float deltaTsl; // difference between mean air temperature for 1961-90 and mean air temperature at sea level
    float aspect; // aspect
    float sw; // shortwave
...
class ProcessModel
   ...
   void treeType(float everg, float needl, BaseCalibrationEnum baseCalib, 
