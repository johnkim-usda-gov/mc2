...
bool CENTURY_BiogeochemModel::runModelAndCollectData(const int year_index, const int row_index, const int col_index)
{
	...
	switch (runParamsP->runMode)
	{
		case CENTURY_EQ:
			...
			for (int i = 0; i<NUM_EQ_OUTVARS; i++) if (pS->EQandWSoutputList[i].show) 
			{ float floatval;
		...
		case SPINUP:
			assert(NUM_WS_OUTVARS<CEN_LENGTH);
			for (int i = 0; i<CEN_LENGTH; i++) EQdata.cenOutvars[i] = *(input_data.inFromEQ + i);
		...
		case TRANSIENT:
			assert(NUM_WS_OUTVARS<CEN_LENGTH);
			for (int i = 0; i<CEN_LENGTH; i++) EQdata.cenOutvars[i] = *(input_data.inFromWS + i);
...
// Simulate one point for one year.
{
	...
	if (runParamsP->fireModelFlag) 
	{
		...
		possibleFireFlag = (runParamsP->fire_set_interval>0 && runParamsP->fire_set_interval<=m_burn_count)
			|| (pF->m_ffmc_ann_max>=pF->m_ffmc_threshold && pF->m_bui_ann_max>=pF->m_bui_threshold);               
	}
	else
	{
	...
	{ 
		...
		{ 
			...
			{ // no fire so far this year; should a fire be simulated this month?
				...
				if (!possibleFireFlag || not_enough_C) fire_dom = NO_FIRE;
	...
	treeType(everg, needl, runParamsP->baseCalibration, tree_typP, &broadleafFlag, &deciduousFlag);
	...
	switch (runParamsP->baseCalibration)
	{
		/*
		   ...
		   case mc2CA08: 
		   case mc2YOSE:
		   assert(0); // NewBiogeogCA08(data_point, mapss_output); 
		   break;
		   case mc2VINCERA:
		   assert(0); // BiogeogVincera(data_point, mapss_output);
		   break;
		   case mc2US50km:
		   assert(0); // BiogeogUS50km();
		   break;
		 */
		case mc2WCR:
		...
		case mc2GLOBAL:
		case mc2California:
			pB->BiogeogMC2(inVals, &m_biome, &m_physiognomic_class, &mc2vtype);
			*vtypeP = (int)mc2vtype;
			break;
		case mc2ConUS_LC:
	...
	pS->VarDict[PSL].save(pB->m_psl, YEAR_INTERVAL);
	pS->VarDict[SSZ_UB].save(pB->m_SSZupperBound, YEAR_INTERVAL);
	pS->VarDict[PSFZ_LB].save(pB->m_PSFZlowerBound, YEAR_INTERVAL);
	pS->VarDict[SAFZ_LB].save(pB->m_SAFZlowerBound, YEAR_INTERVAL);
	pS->VarDict[PKLZ_LB].save(pB->m_PKLZlowerBound, YEAR_INTERVAL);
...
bool CENTURY_BiogeochemModel::mc2_transient()
{
	...
	float ppt_smoothed[12], tmp_smoothed[12];
	...
	treeType(initial_everg, initial_needl, runParamsP->baseCalibration,
			&initial_tree_typ, &stateData.deciduous, &stateData.broadleaf);
	...
	if (modelParamsP->code_flags[NLAYER_LE_9_FLAG] && depth>2100.) depth = 2100.;
	...
	for (int yrNdx = 0; yrNdx<runParamsP->actual_years_to_run; yrNdx++) 
		simulate1pt1yr(&fmi, &biogeogModelInstance, &prev_vclass, &vtype, &tree_typ, yrNdx, yrNdx,
				cen_state, tmp_smoothed, ppt_smoothed, initial_tmp_index, initial_ppt_index);

	cen_end_(); // Century closes files 

	for (int i = 0; i<CEN_LENGTH; i++) stateData.eqState.cenOutvars[i] = EQdata.cenOutvars[i];
	...
	if (runParamsP->multiyrOutputFlag && pS->m_num_multiyr_vars>0) pS->saveMultiyrOutputData();
...
bool CENTURY_BiogeochemModel::mc2_spinup()
{
	...
	/* Initialize climate smoothing for ppt and tmp. */
	tau = modelParamsP->efold_t_max;
	...
	for (spinup_yr = 0; spinup_yr<runParamsP->actual_years_to_run; spinup_yr += runParamsP->years_of_climate_data) 
	{
		for (climate_yr = 0; climate_yr<runParamsP->years_of_climate_data; climate_yr++) simulate1pt1yr(
				&fmi, &biogeogModelInstance, &prev_vclass, &vtype, &tree_typ, 
				(spinup_yr + climate_yr), climate_yr, cen_state,
				tmp_smoothed, ppt_smoothed, initial_tmp_index, initial_ppt_index);
...
{
	float ppt_raw[12], tmax_raw[12], tmin_raw[12], tmp_raw[12], vpr_raw[12], rh_raw[12], pet[12];
	float tmp_index, ppt_index, mix_index;

	for (int mo = 0; mo<12; mo++)
	{ 
		//source_mo = climateYrNdx * 12 + (modelParamsP->southernHemisphereFlag ? (mo + 6) : mo);
		int source_mo = climateYrNdx * 12 + mo;
		if (modelParamsP->southernHemisphereFlag)
			source_mo += 6;
	...
	/* Calculate this year's smoothed tmp & ppt values, and use them to calculate 
	   tmp_index, ppt_index, mix_index, and c3c4_index, which will be passed in to cen_step(). */
	for (int mo = JAN; mo <= DEC; mo++) 
	{
