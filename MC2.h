...
typedef struct 
   ...
   size_t row_dimlen, col_dimlen, time_dimlen; // ws_dimlen is NUM_WS_OUTVARS
   int vardimids[4];
...
// routines called by other methods
   ...
   void getVarInfo(ncid_type * ncidP); // Calls getAttInfo(). 1X
   ...
   void initializeDiscretionaryOutputFile(ncid_type * ncidP, const char * suffix, OutVarDescription outList[], int num_vars,
         char * cmd_line, char * cmd_file_name); // 4X planned
   ...
   void writeStdAttributes(ncid_type * ncidP); // 3X
...
// Member Data
   ...
   // Next 6 are for WCR base calibration
